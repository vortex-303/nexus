...
func (s *Server) handleCreateDoc(w http.ResponseWriter, r *http.Request) {
	...
	_, err = wdb.DB.Exec(
		...
		docID, req.Title, req.Content, claims.UserID, claims.UserID, now, now,
	)
	if err != nil {
...
func (s *Server) handleListDocs(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := wdb.DB.Query(
		"SELECT id, title, content, created_by, COALESCE(updated_by,''), sharing, COALESCE(channel_id,''), created_at, updated_at FROM documents ORDER BY updated_at DESC",
	)
	if err != nil {
	...
	for rows.Next() {
		var d docResp
		if err := rows.Scan(&d.ID, &d.Title, &d.Content, &d.CreatedBy, &d.UpdatedBy, &d.Sharing, &d.ChannelID, &d.CreatedAt, &d.UpdatedAt); err != nil {
			continue
		}
		docs = append(docs, d)
	}
...
func (s *Server) handleGetDoc(w http.ResponseWriter, r *http.Request) {
	...
	var d docResp
	err = wdb.DB.QueryRow(
		"SELECT id, title, content, created_by, COALESCE(updated_by,''), sharing, COALESCE(channel_id,''), created_at, updated_at FROM documents WHERE id = ?",
		docID,
	).Scan(&d.ID, &d.Title, &d.Content, &d.CreatedBy, &d.UpdatedBy, &d.Sharing, &d.ChannelID, &d.CreatedAt, &d.UpdatedAt)
	if err != nil {
